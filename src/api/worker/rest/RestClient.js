...
export class RestClient {
	...
	request(path: string, method: HttpMethodEnum, queryParams: Params, headers: Params, body: ?string | ?Uint8Array, responseType: ?MediaTypeEnum, progressListener: ?ProgressListener): Promise<any> {
		if (this._clientSuspended) {
			return this._deferRequest(() => this.request(path, method, queryParams, headers, body, responseType, progressListener))
		} else {
				...
				xhr.onload = () => { // XMLHttpRequestProgressEvent, but not needed
					...
					if (xhr.status === 200 || method === HttpMethod.POST && xhr.status === 201) {
						...
						if (xhr.status === 429 && retryAfter) {
							...
							if (!this._clientSuspended) {
								console.log(`client suspension activated:  ${suspensionTime}s`)
								this._clientSuspended = true
								const suspentionStartTime = Date.now()
								setTimeout(() => {
									console.log(`client suspension released after ${(Date.now() - suspentionStartTime) / 1000}s`)
									this._clientSuspended = false
								...
								}, Number(suspensionTime) * 1000)
							} else {
								console.log(`client request in suspended state ${suspensionTime}s`)
							}
							resolve(this._deferRequest(() => this.request(path, method, queryParams, headers, body, responseType, progressListener)))
						} else {
	...
	_deferRequest(request: () => Promise<any>): Promise<any> {
